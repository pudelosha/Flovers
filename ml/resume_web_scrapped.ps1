...
$logFile = Join-Path $logDir "resume_web_scrapped_$ts.log"
$errFile = Join-Path $logDir "resume_web_scrapped_$ts.err.log"
...
Write-Host "[INFO] Resuming training from: $ML_DIR"
Write-Host "[INFO] STDOUT log: $logFile"
Write-Host "[INFO] STDERR log: $errFile"
Write-Host "[INFO] Command: python -u train_full.py --resume"
Write-Host ""

$psi = @{
    FilePath               = (Get-Command python).Source
    ArgumentList           = @("-u", "train_full.py", "--resume")
...
Write-Host ""
Write-Host "[DONE] Resume process ended. Logs in: $logDir" -ForegroundColor Cyan
