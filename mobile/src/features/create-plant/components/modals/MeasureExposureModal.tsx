...
export default function MeasureExposureModal({
  ...
  const [finalOrientation, setFinalOrientation] =
  ...
  const endTest = useCallback(() => {
    ...
    if (hs.length) {
      const meanRad = Math.atan2(
        hs.reduce((sum, d) => sum + Math.sin((d * Math.PI) / 180), 0),
        hs.reduce((sum, d) => sum + Math.cos((d * Math.PI) / 180), 0)
      );
  ...
  const lightLabelLive = useMemo(() => {
    if (Platform.OS !== "android") {
      return tr("createPlant.step04.modal.notAvailable", "Not available");
    }

    if (lux == null) {
      return tr("createPlant.step04.modal.noSensor", "No sensor");
    }
    ...
    <>
      ...
      <View style={remindersStyles.promptWrap}>
        ...
        >
            ...
            <View style={{ gap: 10 }}>
              ...
              <View style={[rowStyle, { borderBottomWidth: 0 }]}>
                ...
                <Text style={rowVal}>
                  {isTestRunning
                    ? `~${countdownSec}s`
                    : tr("createPlant.step04.modal.done", "done")}
                </Text>
              </View>

              <View style={{ paddingVertical: 2 }}>
                <Text style={styles.resultText}>
                  {tr("createPlant.step04.modal.bestLight", "Light (best in 5s)")}{" "}
                  {lightLabelFinal}
                </Text>

                <Text style={[styles.resultText, { marginTop: 4 }]}>
                  {tr(
                    "createPlant.step04.modal.meanDirection",
                    "Direction (mean in 5s)"
                  )}{" "}
            ...
            <View style={styles.buttonsRow}>
              <Pressable style={wiz.btn} onPress={onClose}>
                <Text style={wiz.btnText}>
                  {tr("createPlant.step04.modal.close", "Close")}
                </Text>
              </Pressable>
              ...
              <Pressable
                style={[
                  wiz.btn,
                  wiz.btnPrimary,
                  !canApply ? { opacity: 0.5 } : undefined,
                ]}
                onPress={apply}
                disabled={!canApply}
              >
                <Text style={wiz.btnText}>
                  {tr("createPlant.step04.modal.apply", "Apply")}
                </Text>
              </Pressable>
            </View>
          ...
          <ModalCloseButton
            onPress={onClose}
            accessibilityLabel={tr("createPlant.step04.modal.close", "Close")}
            style={{
              top: 8,
              right: 8,
            }}
